...
for asset,version in [('dashboard-v15','15'),('bills-v16','16'),('crud-v17','17'),('procurement-v18','18'),('operations-v19','19'),('production-v20','20'),('ux-v21','21'),('live-v23','23'),('admin-users-v24','24'),('premium-v26','26'),('standard-v29','29'),('design-tokens','2'),('hierarchy-v32','32')]:
...
if not core_path.exists(): raise SystemExit('assets/core-v14.js is missing')
core=core_path.read_text(encoding='utf-8')
for required_core in ['window.show=function(view)','window.boot=async function(session)',"new Set(['dashboard','bills','new','products','vendors','prices','reports','settings'])"]:
    if required_core not in core: raise SystemExit(f'core-v14.js missing required routing contract: {required_core}')
controller=Path('assets/app-controller-v51.js').read_text(encoding='utf-8')
for required_controller in ['queryAllBills','loadingPromise','applyAuthenticatedUser','resolveRole','window.reloadBillsNow','app-controller']:
...
print('Applied stable Bills app with admin role resolution, display-only health, and single data controller v53.')