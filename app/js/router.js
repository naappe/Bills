...
import {productsPage} from './products.js?v=4.9.63';
...
const meta={dashboard:['Dashboard','Procurement performance overview'],bills:['Bills','Search, review and manage supplier purchases'],new:['Bill entry','Record or edit every item in one supplier bill'],products:['Products','Automatic product catalogue with price history'],rates:['Price Intelligence','Compare normalized product and supplier rates'],prices:['Price Intelligence','Compare normalized product and supplier rates'],vendors:['Vendors','Supplier directory and spend intelligence'],reports:['Reports','Procurement analytics'],settings:['Settings','Workspace preferences and account security'],admin:['Admin & users','Account access, roles and system health']};
let started=false;
function escapeHtml(value){return String(value??'').replace(/[&<>"']/g,character=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[character]))}
function ensurePageHeader(content,title,subtitle){if(content.querySelector(':scope > .page-head'))return;const header=document.createElement('header');header.className='page-head';header.innerHTML=`<div><h1>${escapeHtml(title)}</h1><p>${escapeHtml(subtitle)}</p></div>`;content.prepend(header)}
