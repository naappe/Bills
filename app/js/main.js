...
const health={version:'4.9.1',booted:false,authenticated:false,dataLoaded:false,error:null,startedAt:new Date().toISOString()};
...
function showWorkspaceError(error){
  ...
  console.error('[Procurement workspace]',error);
  $('#content').innerHTML=`<section class="panel"><div class="empty"><i class="fa-solid fa-triangle-exclamation"></i><h2>Workspace could not finish loading</h2><p>${message.replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}</p><button class="btn" id="retryWorkspace" type="button">Retry</button></div></section>`;
