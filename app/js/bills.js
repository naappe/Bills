...
export function billsPage(){
  ...
  const draw=()=>{
    ...
    $('#billRows').innerHTML=slice.map(entry=>{const {row,numberLabel,vendorName}=entry;const entered=formatDateTime(get(row,'created_at'));const numberText=billNo(row)==='—'?'No bill number':`Bill ${billNo(row)}`;return`<tr class="bill-row-click" data-view="${row.id}" tabindex="0"><td class="bill-record"><strong>${escapeHtml(numberLabel)}</strong><small class="cell-meta">${escapeHtml(numberText)}</small></td><td><strong>${escapeHtml(entry.date||'No date')}</strong></td><td class="bill-entry-time"><strong>${escapeHtml(entered)}</strong></td><td class="bill-vendor"><strong>${escapeHtml(vendorName)}</strong><small class="cell-meta">${escapeHtml(numberText)}</small></td><td class="num bill-total"><strong>${money(amount(row))}</strong></td><td class="action-col"><div class="actions">${canEdit(row)?`<button class="btn secondary small" data-edit="${row.id}">Edit</button>`:'<span class="locked">Locked</span>'}${store.role==='admin'?`<button class="btn danger small" data-delete="${row.id}">Delete</button>`:''}</div></td></tr>`}).join('')||'<tr><td colspan="6" class="empty">No bills match these filters.</td></tr>';
