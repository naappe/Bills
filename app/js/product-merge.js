...
function draw(){
 ...
 $('#pmSourceColumn').classList.toggle('chosen',!!source);$('#pmTargetColumn').classList.toggle('chosen',!!target);$('#pmMerge').disabled=!(source&&target);$('#pmMerge').textContent=source&&target?`Merge into ${target.name}`:'Select both products';
...
export async function productMergePage(){
 ...
 products=data||[];
 $('#content').innerHTML=`<section class="pm-shell"><header class="pm-top"><div><h2>Merge master products</h2><p>Select the duplicate product and move everything to the correct master product.</p></div><button class="btn secondary" id="pmBack">Back to Products</button></header><div class="pm-columns"><section class="pm-column" id="pmSourceColumn"><div class="pm-title"><span class="pm-step">1</span><div><h3>Wrong product</h3><small>This duplicate name will be removed</small></div></div><input class="pm-search" id="pmSourceSearch" placeholder="Search wrong product"><div class="pm-list" id="pmSourceList"></div></section><div class="pm-arrow"><span aria-hidden="true"></span></div><section class="pm-column" id="pmTargetColumn"><div class="pm-title"><span class="pm-step">2</span><div><h3>Correct master product</h3><small>All bills, prices and image will use this product</small></div></div><input class="pm-search" id="pmTargetSearch" placeholder="Search or write correct product name"><div class="pm-list" id="pmTargetList"></div></section></div><footer class="pm-footer"><div id="pmSummary"></div><button class="btn danger" id="pmMerge" disabled>Merge selected products</button></footer></section>`;
 $('#pmBack').onclick=()=>location.hash='#products';$('#pmSourceSearch').oninput=draw;$('#pmTargetSearch').oninput=()=>{target=null;draw()};
 $('#content').addEventListener('click',event=>{const item=event.target.closest('[data-side]');if(!item)return;const product=products.find(row=>row.id===item.dataset.id);if(item.dataset.side==='source'){source=product;if(target?.id===product.id)target=null;$('#pmTargetSearch').focus()}else{target=product;$('#pmMerge').focus()}draw()});
