...
const helperAreas=[
  ...
  ['KPI Summary','.dashboard-metrics,.bills-summary,.grid-4,.cost-kpis,.admin-summary-grid'],
  ['KPI Cards','.dashboard-metric,.kpi,.cost-kpis>article,.bills-summary>div'],
...
function installStyles(){
  ...
  style.textContent=`
    ...
    .page-helper-panel{width:min(360px,calc(100vw - 28px));max-height:min(520px,70vh);overflow:auto;padding:14px;border:1px solid #e7c86f;border-radius:16px;background:#fffdf7;box-shadow:0 18px 48px rgba(15,42,69,.22)}
...
function installPageHelper(root=document.querySelector('#content')){
  ...
  const existing=$('#globalPageHelper');
  existing?.remove();
