...
function applyOverrides(){
  const data=read();
  document.querySelectorAll('.audit-product,.pr-price-card,.pr-product-card').forEach(card=>{
    ...
    const category=card.querySelector('.audit-product>div>span,.pr-product-visual small');
    ...
    if(!card.querySelector('[data-edit-product]')){
      ...
      (card.querySelector('.audit-product>div,.pr-product-body,footer')||card).appendChild(btn);
...
function assetAudit(){
  const rows=[];
  document.querySelectorAll('script[src]').forEach(x=>{const file=x.src.split('/').pop(),name=file.split('?')[0];rows.push({type:'script',file,layer:/site-audit-v7|product-editor-v8/.test(name)?'current override':/procurement-pages-v4|recovery-v5|vendors-settings-v6|admin-manage-v5|bills-period-filter-v5/.test(name)?'transition override':/view-renderers|rates-page|settings-page|vendor-/.test(name)?'legacy/base':'core or vendor'})});
  document.querySelectorAll('link[rel="stylesheet"][href]').forEach(x=>{const file=x.href.split('/').pop(),name=file.split('?')[0];rows.push({type:'style',file,layer:/site-audit-v7|product-editor-v8/.test(name)?'current override':/procurement-pages-v4|recovery-v5|vendors-settings-v6|admin-manage-v5/.test(name)?'transition override':/procurement-ui|ui-structure|alignment-system|procurement-rebuild-v3/.test(name)?'legacy/base':'vendor'})});
  console.groupCollapsed('[asset-audit] loaded application assets');console.table(rows);console.info('Renderer flags',{v3:!!window.__WS_PROCUREMENT_REBUILD__,v4:!!window.__WS_PROCUREMENT_PAGES_V4__,v5:!!window.__WS_RECOVERY_V5__,v6:!!window.__WS_VENDORS_SETTINGS_V6__,v7:!!window.__WS_SITE_AUDIT__,v8:true});console.groupEnd();return rows;
}
window.WSAssetAudit=assetAudit;window.__WS_PRODUCT_EDITOR__={version:8,storage:KEY,apply:applyOverrides};
