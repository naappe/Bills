...
window.renderDashboard=()=>{
 const list=all(),total=list.reduce((sum,row)=>sum+amountVal(row),0),monthRows=filterDates(list,'month');
 const paid=list.filter(row=>statusVal(row).toLowerCase()==='paid').reduce((sum,row)=>sum+amountVal(row),0);
 const pending=Math.max(0,total-paid),supplierTotals=new Map();
 list.forEach(row=>{const supplier=vendorVal(row)||'Unknown supplier';supplierTotals.set(supplier,(supplierTotals.get(supplier)||0)+amountVal(row))});
 const suppliers=[...supplierTotals.entries()].sort((a,b)=>b[1]-a[1]).slice(0,5);
 const todayDate=new Date(),months=[];for(let offset=5;offset>=0;offset--){const date=new Date(todayDate.getFullYear(),todayDate.getMonth()-offset,1),key=date.toISOString().slice(0,7);months.push({key,label:date.toLocaleDateString('en-US',{month:'short'})})}
 const trend=months.map(month=>list.filter(row=>iso(dateVal(row)).startsWith(month.key)).reduce((sum,row)=>sum+amountVal(row),0));
 const rateProducts=new Set();list.forEach(bill=>(bill.items||[]).forEach(item=>{const name=text(val(item,'product','description'));if(name)rateProducts.add(name.toLowerCase())}));
 const page=head('Dashboard','Procurement overview')+(canAdd()?'<button class="btn" data-go="bills">Add bill</button>':'')+
 '<section class="dashboard-kpis"><article class="dashboard-kpi"><span>Total procurement</span><strong>'+money(total)+'</strong><small>'+list.length.toLocaleString()+' bills recorded</small></article><article class="dashboard-kpi dashboard-kpi-brand"><span>This month</span><strong>'+money(monthRows.reduce((sum,row)=>sum+amountVal(row),0))+'</strong><small>'+monthRows.length+' bills this month</small></article><article class="dashboard-kpi dashboard-kpi-warning"><span>Pending payment</span><strong>'+money(pending)+'</strong><small>'+list.filter(row=>statusVal(row).toLowerCase()!=='paid').length+' pending bills</small></article><article class="dashboard-kpi dashboard-kpi-green"><span>Active suppliers</span><strong>'+supplierTotals.size+'</strong><small>'+rateProducts.size+' products with rate history</small></article></section>'+
 '<section class="dashboard-grid"><article class="analytics-card analytics-chart-card"><div class="analytics-head"><div><h2>Purchase trend</h2><p>Monthly procurement value</p></div><span class="analytics-period">Last 6 months</span></div><div class="chart-wrap"><canvas id="spendTrendChart"></canvas></div></article><article class="analytics-card"><div class="analytics-head"><div><h2>Top suppliers</h2><p>By total purchase value</p></div><button class="text-action" data-go="bills">View bills</button></div><div class="supplier-rankings">'+(suppliers.length?suppliers.map((entry,index)=>'<div class="supplier-rank"><b>'+String(index+1).padStart(2,'0')+'</b><span>'+esc(entry[0])+'</span><strong>'+money(entry[1])+'</strong></div>').join(''):'<div class="empty">No supplier data yet.</div>')+'</div></article><article class="analytics-card analytics-payment-card"><div class="analytics-head"><div><h2>Payment status</h2><p>Current recorded value</p></div></div><div class="payment-summary"><div><span>Paid</span><strong>'+money(paid)+'</strong></div><div><span>Pending</span><strong>'+money(pending)+'</strong></div></div><div class="payment-bar"><i style="width:'+(total?Math.max(0,Math.min(100,(paid/total)*100)):0)+'%"></i></div></article><article class="analytics-card analytics-rate-card"><div class="analytics-head"><div><h2>Rate intelligence</h2><p>Prices saved from bill items</p></div><button class="text-action" data-go="rates">Open rates</button></div><strong class="rate-analytics-value">'+rateProducts.size+'</strong><span class="muted">Products ready for supplier comparison</span></article></section>';
 byId('content').innerHTML=page;
 if(window.Chart&&byId('spendTrendChart')){
  try{window.__WS_DASHBOARD_CHART__?.destroy();window.__WS_DASHBOARD_CHART__=new Chart(byId('spendTrendChart'),{type:'bar',data:{labels:months.map(month=>month.label),datasets:[{data:trend,backgroundColor:'#1677ff',borderRadius:8,borderSkipped:false,maxBarThickness:42}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:context=>money(context.raw)}}},scales:{x:{grid:{display:false},border:{display:false}},y:{grid:{color:'rgba(146,166,190,.22)'},border:{display:false},ticks:{callback:value=>'MVR '+Number(value).toLocaleString()}}}}})}catch(error){console.warn('Dashboard chart unavailable',error)}
 }
};
...
window.renderAdmin=async()=>{
 if(state.role!=='admin'){byId('content').innerHTML=head('Admin','Access restricted')+'<section class="card"><div class="empty">Administrator access is required.</div></section>';return}
 byId('content').innerHTML=head('Admin & users','Manage roles, access, and recent activity')+'<section class="admin-loading card"><div class="empty">Loading user management…</div></section>';
 try{
  ...
  const userCard=user=>'<article class="user-management-card" data-user="'+esc(user.user_id||'')+'"><div class="user-management-main"><div class="user-initial">'+esc((user.display_name||user.email||'?').slice(0,1).toUpperCase())+'</div><div><strong>'+esc(user.display_name||user.email||'Unnamed user')+'</strong><small>'+esc(user.email||'No email')+'</small></div></div><div class="user-management-fields"><label>Role<select class="field" data-role>'+['admin','manager','staff','readonly'].map(role=>'<option value="'+role+'" '+(role===text(user.role).toLowerCase()?'selected':'')+'>'+role+'</option>').join('')+'</select></label><label>Status<select class="field" data-active><option value="true" '+(user.is_active!==false?'selected':'')+'>Active</option><option value="false" '+(user.is_active===false?'selected':'')+'>Disabled</option></select></label></div><div class="user-management-meta"><span>'+esc(user.current_view||'No recent page')+'</span><span>'+esc(user.last_seen_at?new Date(user.last_seen_at).toLocaleString():'No activity')+'</span></div><button class="btn secondary small" data-save-user>Save changes</button></article>';
  byId('content').innerHTML=head('Admin & users','Manage roles, access, and recent activity')+'<section class="dashboard-kpis admin-kpis"><article class="dashboard-kpi"><span>Users</span><strong>'+users.length+'</strong><small>All registered users</small></article><article class="dashboard-kpi dashboard-kpi-green"><span>Online now</span><strong>'+online+'</strong><small>Seen in the last 10 minutes</small></article><article class="dashboard-kpi dashboard-kpi-brand"><span>Administrators</span><strong>'+users.filter(user=>text(user.role).toLowerCase()==='admin').length+'</strong><small>Full access accounts</small></article><article class="dashboard-kpi dashboard-kpi-warning"><span>Disabled</span><strong>'+users.filter(user=>user.is_active===false).length+'</strong><small>Accounts without access</small></article></section><section class="user-management-grid">'+(users.length?users.map(userCard).join(''):'<div class="empty card">No user records returned.</div>')+'</section>';
  byId('content').querySelectorAll('[data-save-user]').forEach(button=>button.onclick=async()=>{const card=button.closest('[data-user]');if(!card)return;button.disabled=true;button.textContent='Saving…';const update=await db.rpc('admin_update_user_role',{target_user:card.dataset.user,new_role:card.querySelector('[data-role]').value,new_active:card.querySelector('[data-active]').value==='true',new_display_name:null});button.disabled=false;if(update.error){button.textContent='Retry';alert('User update failed: '+update.error.message);return}button.textContent='Saved';setTimeout(()=>button.textContent='Save changes',1200)});
 }catch(error){byId('content').innerHTML=head('Admin & users','User management is unavailable')+'<section class="card"><div class="empty">The admin user-management service did not respond: '+esc(error.message||String(error))+'</div></section>'}
};
