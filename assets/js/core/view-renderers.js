...
window.renderMobileDemo=()=>{
 const frameUrl=view=>location.pathname+'?mobile-preview=1#'+view;
 byId('content').innerHTML=head('Responsive layout demo','Inspect the real application at phone and tablet widths')+'<section class="mobile-demo"><div class="mobile-demo-controls" role="tablist"><button class="btn secondary small active" type="button" data-mobile-view="new">New Bill</button><button class="btn secondary small" type="button" data-mobile-view="bills">Bills</button><button class="btn secondary small" type="button" data-mobile-view="rates">Rates</button></div><div class="mobile-demo-controls" aria-label="Preview device"><button class="btn secondary small active" type="button" data-device="phone">Phone · 390px</button><button class="btn secondary small" type="button" data-device="tablet">Tablet · 768px</button></div><div class="device-frame phone-frame" id="mobileDeviceFrame"><div class="phone-speaker"></div><iframe id="mobileDemoFrame" title="White Saffron responsive preview" src="'+esc(frameUrl('new'))+'"></iframe></div></section>';
 const frame=byId('mobileDemoFrame'),deviceFrame=byId('mobileDeviceFrame');
 document.querySelectorAll('[data-mobile-view]').forEach(button=>button.onclick=()=>{document.querySelectorAll('[data-mobile-view]').forEach(item=>item.classList.toggle('active',item===button));frame.src=frameUrl(button.dataset.mobileView)});
 document.querySelectorAll('[data-device]').forEach(button=>button.onclick=()=>{document.querySelectorAll('[data-device]').forEach(item=>item.classList.toggle('active',item===button));const tablet=button.dataset.device==='tablet';deviceFrame.classList.toggle('tablet-frame',tablet);deviceFrame.classList.toggle('phone-frame',!tablet)});
};
