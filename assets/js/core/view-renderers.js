...
window.renderNewBill=async()=>{
 ...
 const editing=state.editing||null,vendors=await loadVendors();state.items=Array.isArray(editing?.items)&&editing.items.length?editing.items.map(itemFromSaved):[emptyItem()];
...
window.renderMobileDemo=()=>{
 ...
 byId('content').innerHTML=head('Mobile demo','Preview the actual responsive application at phone width')+`<section class="mobile-demo"><div class="mobile-demo-controls" role="tablist"><button class="btn secondary small active" type="button" data-mobile-view="new">New Bill</button><button class="btn secondary small" type="button" data-mobile-view="bills">Bills</button><button class="btn secondary small" type="button" data-mobile-view="rates">Rates</button></div><div class="phone-frame"><div class="phone-speaker"></div><iframe id="mobileDemoFrame" title="White Saffron mobile preview" src="${esc(frameUrl('new'))}"></iframe></div></section>`;
 const frame=byId('mobileDemoFrame');document.querySelectorAll('[data-mobile-view]').forEach(button=>button.onclick=()=>{document.querySelectorAll('[data-mobile-view]').forEach(item=>item.classList.toggle('active',item===button));frame.src=frameUrl(button.dataset.mobileView)});
};
