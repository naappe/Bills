...
window.renderRates=()=>{
 ...
 if(!content)return;
 content.innerHTML=pageHead('Rates','Supplier purchase comparison and lowest recorded prices')+overview+(cards?`<section class="rate-product-grid">${cards}</section>`:'<section class="card"><div class="empty">Bills are loaded, but no product and rate fields were found in the saved records.</div></section>');
};
if(window.__WS_RENDERERS__)window.__WS_RENDERERS__.rates=window.renderRates;
window.__WS_RATES_COMPAT__={version:2};
})();