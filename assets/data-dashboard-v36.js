...
window.renderBills=function(){
 ...
 <section class="metrics bills-period-totals bills-period-totals-compact">
 ...
 <section class="card bills-card"><div class="toolbar"><input class="field" id="billSearch" placeholder="Search vendor, product, pack, bill number, notes, user or amount"><select class="field" id="billPeriod"><option value="month">This month</option><option value="latest">Latest added</option><option value="3months">Last 3 months</option><option value="year">This year</option><option value="future">Future bill dates</option><option value="all">All records</option><option value="archived">Archived</option></select><select class="field" id="billSize"><option value="20">20 per page</option><option value="50">50 per page</option><option value="100">100 per page</option></select></div><div class="table-wrap"><table><thead><tr><th>Bill date</th><th>Added at</th><th>Bill no.</th><th>Vendor</th><th>Amount</th><th>Created by</th><th>Actions</th></tr></thead><tbody id="billRowsV36"></tbody></table></div><div class="pager" id="billPagerV36"></div></section>`;
 bindGo();
 $('#billSearch').value=saved.q||'';$('#billPeriod').value='month';$('#billSize').value=String(saved.size||20);
 write({...saved,period:'month'});
 let timer;$('#billSearch').addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(()=>{state.page=1;draw()},120)});['billPeriod','billSize'].forEach(id=>$('#'+id).addEventListener('change',()=>{state.page=1;draw()}));$('#billCsv').onclick=exportCsv;draw();
 function draw(){
  const q=$('#billSearch').value.trim().toLowerCase(),period=$('#billPeriod').value;state.pageSize=Number($('#billSize').value||20);write({...read(),q,period,size:state.pageSize});
  ...
  rows=rows.filter(r=>!q||billSearchText(r).includes(q));
  ...
  $('#billRowsV36').innerHTML=shown.map(r=>`<tr class="${isFuture(r)?'future-bill-row':''}"><td>${formatDate(dateVal(r))}${isFuture(r)?'<span class="pill pending" style="margin-left:6px">Future</span>':''}</td><td>${formatDateTime(get(r,'created_at','createdAt','added_at','inserted_at'))}</td><td>${esc(get(r,'bill_no','Bill No')||'-')}</td><td><strong>${esc(vendorVal(r)||'-')}</strong></td><td><strong>${money(amountVal(r))}</strong></td><td>${esc(addedBy(r))}</td><td><div class="actions">${archived?`<button class="btn secondary small" data-restore36="${r.id}">Restore</button>`:`<button class="btn secondary small" data-edit36="${r.id}">Edit</button><button class="btn secondary small" data-archive36="${r.id}">Archive</button>${isAdmin()?`<button class="btn danger small" data-delete36="${r.id}">Delete</button>`:''}`}</div></td></tr>`).join('')||'<tr><td colspan="7"><div class="empty">No bills found for this filter.</div></td></tr>';
...
window.__WS_DATA_FIX__={version:40,billDate};
})();