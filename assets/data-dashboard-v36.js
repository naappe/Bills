...
const stamp=r=>billDate(dateVal(r))?.getTime()||0;
...
const sorted=rows=>[...rows].sort((a,b)=>stamp(b)-stamp(a)||Number(b.id||0)-Number(a.id||0));
...
const periodRows=(rows,period)=>{if(period==='all')return rows;const [a,b]=periodRange(period);return rows.filter(r=>{const t=stamp(r);return t&&t>=a.getTime()&&t<=b.getTime()})};
...
window.renderBills=function(){
 ...
 $('#content').innerHTML=pageHead('Bills','All bill records with reliable imported-date filtering.',actions)+`
 ...
 function draw(){
  ...
  let rows=sorted(state.rows);const archived=period==='archived';rows=rows.filter(r=>archived?!active(r):active(r));
  if(period==='latest')rows=rows.slice(0,100);else if(!['all','archived'].includes(period))rows=periodRows(rows,period);
...
window.renderDashboard=function(){
 ...
 function draw(){const period=$('#dashboardPeriod').value,all=state.rows.filter(active),rows=periodRows(all,period),year=periodRows(all,'year'),labels={month:'This month spend','3months':'Last 3 months spend',year:'This year spend',all:'All-time spend'};write({...read(),dashboardPeriod:period});$('#dashPeriodLabel').textContent=labels[period];$('#dashPeriodTotal').textContent=money(total(rows));$('#dashYearTotal').textContent=money(total(year));$('#dashPeriodCount').textContent=rows.length.toLocaleString();$('#dashAllCount').textContent=all.length.toLocaleString();const vendors=aggregateVendors(rows).slice(0,8),recent=sorted(rows).slice(0,10);$('#dashVendors').innerHTML=vendors.map(v=>`<div class="rank-row"><div><b>${esc(v.name)}</b><br><span>${v.count} bills</span></div><strong>${money(v.total)}</strong></div>`).join('')||'<div class="empty">No bills in this period.</div>';$('#dashRecent').innerHTML=recent.map(r=>`<div class="rank-row"><div><b>${esc(vendorVal(r)||'Unknown vendor')}</b><br><span>${formatDate(dateVal(r))}</span></div><strong>${money(amountVal(r))}</strong></div>`).join('')||'<div class="empty">No recent bills.</div>'}
};
window.__WS_DATA_FIX__={version:36,billDate};
})();