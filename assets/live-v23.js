...
function remember(view){if(!views.has(view))return;localStorage.setItem(VIEW_KEY,view);if(location.hash!==`#${view}`)history.replaceState(null,'',`#${view}`)}
...
let heartbeatTimer=null,lastHeartbeat=0,heartbeatInFlight=false;
async function heartbeat(online=true,force=false){
  if(!state.user||heartbeatInFlight)return;
  const now=Date.now();if(!force&&now-lastHeartbeat<60000)return;
  heartbeatInFlight=true;lastHeartbeat=now;
  const payload={user_id:state.user.id,email:state.user.email||'',display_name:state.user.user_metadata?.full_name||state.user.email?.split('@')[0]||'User',role:state.role||'staff',current_view:state.view||localStorage.getItem(VIEW_KEY)||'dashboard',is_online:online,last_seen_at:new Date().toISOString(),updated_at:new Date().toISOString()};
  try{await db.from('user_presence').upsert(payload,{onConflict:'user_id'})}catch(err){console.warn('Presence update failed',err)}finally{heartbeatInFlight=false}
}
function startPresence(){clearInterval(heartbeatTimer);heartbeat(true,true);heartbeatTimer=setInterval(()=>heartbeat(true),60000)}
db.auth.onAuthStateChange((event,session)=>{if(session)startPresence();else clearInterval(heartbeatTimer)});
document.addEventListener('visibilitychange',()=>{if(state.user)heartbeat(!document.hidden,true)});
window.addEventListener('pagehide',()=>{if(state.user)heartbeat(false,true)});

let presenceCache={at:0,rows:[]};
async function presenceRows(force=false){if(!isAdmin())return[];if(!force&&Date.now()-presenceCache.at<30000)return presenceCache.rows;const {data}=await db.from('user_presence').select('*').order('last_seen_at',{ascending:false}).limit(30);presenceCache={at:Date.now(),rows:data||[]};return presenceCache.rows}
...
window.renderDashboard=async function(){
  ...
  bindGo();remember('dashboard');$('#v23Refresh').onclick=async()=>{try{await loadBills(true)}catch{}presenceCache.at=0;renderDashboard()};window.__WS_ENHANCE__?.();window.__WS_HIERARCHY__?.apply?.();
};
const originalNew=window.renderNewBill;if(typeof originalNew==='function')window.renderNewBill=async function(){await originalNew();remember('new');const form=document.querySelector('#phaseBillForm');if(form&&!form.querySelector('.v23-sync-note')){const note=document.createElement('div');note.className='v23-sync-note';note.textContent='Saving this bill will automatically update Products and Price Book from its item names, linked products and calculated base-unit rates.';form.prepend(note)}window.__WS_ENHANCE__?.();window.__WS_HIERARCHY__?.apply?.()};
const originalShow=typeof show==='function'?show:null;if(originalShow){window.show=function(view){remember(view);heartbeat(true);const result=originalShow(view);setTimeout(()=>{window.__WS_ENHANCE__?.();window.__WS_HIERARCHY__?.apply?.()},0);return result}}
})();