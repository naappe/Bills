...
function classifyDashboard(){document.querySelectorAll('.metric:not(.stat-card)').forEach(x=>x.classList.add('stat-card'));document.querySelectorAll('.dash-card,.ux-card').forEach(x=>{const text=(x.querySelector('h2,h3,.card-head')?.textContent||'').toLowerCase();if(text.includes('trend')||text.includes('chart')||x.querySelector('canvas,svg'))x.classList.add('chart-card');if(text.includes('activity')||text.includes('latest'))x.classList.add('activity-card')})}
function applyHierarchy(){const content=document.querySelector('#content');if(content){upgradeHeadings(content);classifyDashboard()}addShortcutHelp()}
...
window.__WS_HIERARCHY__={version:33,apply:queue};
})();