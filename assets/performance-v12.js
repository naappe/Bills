(()=>{
  ...
  // Keep navigation state without replacing the application's core show/boot functions.
  document.addEventListener('click',event=>{
    const button=event.target.closest('[data-view],[data-go]');
    const view=button?.dataset.view||button?.dataset.go;
    if(view&&['dashboard','bills','new','products','vendors','prices','reports','settings'].includes(view)){
      localStorage.setItem('ws-current-view',view);
    }
  },true);

  // Restore the last page once, only after the authenticated application is ready.
  let restored=false;
  const restoreTimer=setInterval(()=>{
    if(restored)return clearInterval(restoreTimer);
    const app=document.querySelector('#appView');
    const content=document.querySelector('#content');
    if(!app||app.classList.contains('hidden')||!content||typeof window.show!=='function')return;
    restored=true;
    clearInterval(restoreTimer);
    const saved=localStorage.getItem('ws-current-view');
    if(saved&&saved!=='dashboard'&&['bills','new','products','vendors','prices','reports','settings'].includes(saved)){
      try{window.show(saved)}catch(error){console.error('Page restore failed:',error);localStorage.removeItem('ws-current-view');}
    }
  },120);
  setTimeout(()=>clearInterval(restoreTimer),10000);
  ...
  window.loadBills=async function(loadAll=false){
    ...
    if(error&&!loadAll){
      ({data,error}=await db.from(TABLE).select('*').order('id',{ascending:false}).limit(5000));
      loadAll=true;
    }
    if(error){
      if(typeof originalLoadBills==='function')return originalLoadBills();
      throw error;
    }
    state.rows=data||[];
  ...
  window.renderBills=function(){
    const actions=`<button class="btn secondary" id="exportBtn">Export CSV</button><button class="btn" data-go="new" type="button">＋ New Bill</button>`;
    ...
    ['statusFilter','periodFilter','dateFrom','dateTo','sizeFilter'].forEach(id=>$('#'+id)?.addEventListener('change',async()=>{
      ...
      if(period==='all'&&!state.allBillsLoaded){
        const note=$('#filterNote');if(note)note.textContent='Loading all historical bills…';
  ...
  window.addEventListener('error',event=>console.error('Bills UI error:',event.error||event.message));
})();