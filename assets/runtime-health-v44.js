...
const addHealthCards=()=>{
  ...
  if(!statusCard){
    statusCard=document.createElement('article');
    statusCard.className='metric';
    statusCard.dataset.dbStatus='';
    metrics.appendChild(statusCard);
  }
  ...
  if(!countCard){
    countCard=document.createElement('article');
    countCard.className='metric';
    countCard.dataset.dbCount='';
    metrics.appendChild(countCard);
  }
...
window.checkAppVersions=()=>{
  const status={
    ...
    auth:window.__WS_AUTH__?.version||null,
    controller:window.__WS_APP_CONTROLLER__?.version||null,
    runtime:VERSION,
    connected:Boolean(window.__WS_DB_STATUS__?.connected),
    records:Number(window.__WS_DB_STATUS__?.count||state?.rows?.length||0)
  };
  console.table(status);
  return status;
};

// Database loading and refresh are owned exclusively by app-controller-v51.js.
// This module only renders the controller's current status.
